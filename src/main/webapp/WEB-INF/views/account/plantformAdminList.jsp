		...
		<div class="page-content">
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">					<div class="">						<!-- BEGIN BORDERED TABLE PORTLET-->						<div class="portlet box red">							<div class="portlet-title">								<div class="caption"><i class="icon-user"></i> 平台管理员</div>								<div class="tools">									<a href="${ctx}/admin/plantform-admin/create" class="btn">新增</a>									<a href="javascript:;" class="collapse"></a>								</div>							</div>							<div class="portlet-body">								<table class="table table-striped table-bordered  table-hover">									<thead>										<tr>											<th>#</th>											<th>账号</th>											<th>真实姓名</th>											<th>邮箱</th>											<th>手机</th>											<th class="hidden-480">注册日期</th>											<th>操作</th>										</tr>									</thead>									<tbody>										<c:forEach items="${list}" var="obj" varStatus="status">
										<tr>											<td>${status.count}</td>											<td>${obj.loginName}</td>											<td>${obj.name}</td>											<td>${obj.email}</td>											<td>${obj.mobilephone}</td>											<td class="hidden-480"><fmt:formatDate value="${obj.createDate}" pattern="yyyy-MM-dd"></fmt:formatDate></td>											<td>												<a href="${ctx}/admin/plantform-admin/update/${obj.id}" class="btn mini purple"><i class="icon-edit"></i> 编辑</a>												<a href="javascript:delConfirm('${ctx}/admin/plantform-admin/delete/${obj.id}');" class="btn mini black"><i class="icon-trash"></i> 删除</a>													<a href="${ctx}/admin/plantform-admin/pwd/${obj.id}" class="btn mini blue"><i class="icon-share"></i> 修改密码</a>											</td>
										</tr>										</c:forEach>									</tbody>								</table>
							</div>
